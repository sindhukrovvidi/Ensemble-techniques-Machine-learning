...
library("VIM")
data1 <- kNN(data_processed, variable = c("Gender","Married","Dependents","Self_Employed"), k = 5)
sapply(data1, function(x) sum(is.na(x)))
data1 <- subset(data1, select = Loan_ID:Loan_Status)
#convert <- sapply(data1,is.factor)
#data2<-sapply(data1[,convert],unclass)  
#out<-cbind(data1[,!must_convert],data2) 
index <- createDataPartition(data1$Loan_Status, p=0.75, list=FALSE)
trainSet <- data1[ index,]
testSet <- data1[-index,]
fitControl <- trainControl(method = "cv", number = 5, savePredictions = 'final', classProbs = T)
outcomeName<-'Loan_Status'
predictors<-c("Credit_History", "LoanAmount", "Loan_Amount_Term", "ApplicantIncome","CoapplicantIncome","Education","Self_Employed")
...
data1$Graduate <- sapply(1:length(data1$Education), function(x) if (data1[x,1] %like% "Graduate") 1 else 0)
data1$NotGraduate<- sapply(1:length(data1$Education), function(x) if (data1[x,1] %like% "Not Graduate") 1 else 0)
data1$Syes<- sapply(1:length(data1$Self_Employed), function(x) if (data1[x,1] %like% "Yes") 1 else 0)
data1$Sno<- sapply(1:length(data1$Self_Employed), function(x) if (data1[x,1] %like% "No") 1 else 0)
predictors1 <- c("Credit_History", "LoanAmount", "Loan_Amount_Term", "ApplicantIncome","CoapplicantIncome")
predictors1

model_knn<-train(trainSet[,predictors1],trainSet[,outcomeName],method='knn',trControl=fitControl,tuneLength=3)
testSet$pred_knn<-predict(object = model_knn,testSet[,predictors1])
cm <- confusionMatrix(testSet$Loan_Status,testSet$pred_knn)
a<-cm$overall['Accuracy']
print(a)
model_svm<-train(trainSet[,predictors1],trainSet[,outcomeName],method='svmLinear',trControl=fitControl,tuneLength=3)
testSet$pred_svm<-predict(object = model_svm,testSet[,predictors1])
...
testSet$OOF_pred_knn<-predict(model_knn,testSet[predictors1],type='prob')$Y
testSet$OOF_pred_svm<-predict(model_svm,testSet[predictors1],type='prob')$Y
